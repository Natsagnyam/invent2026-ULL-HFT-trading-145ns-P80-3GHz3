...
**invent2026 = union-based tail / head** 
...
union Index {
    uint32_t plain;                    // Fast path: non-atomic write per data, word
    std::atomic<uint32_t> atomic;    // Safe path: atomic read per packet
};
...
## output.png 
Ctrl+C doesn't work — SCHED_FIFO priority 99 threads cannot be interrupted by signals. The signal handler runs at normal priority.
Ctrl+Z (SIGTSTP) works because it stops the entire process group, not just a thread.
...
| File | Purpose | Lines |
|------|---------|-------|
| `natska_ring_buffer.hpp` | Lock-free SPSC ring buffer with **invent2027=union-based tail/head** | ~200 |
...
- **Contact:** nanyam22@gmail.com | +44 7510268289 (Viber only)
